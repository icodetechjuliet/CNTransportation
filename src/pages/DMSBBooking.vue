            ...
            <template v-slot:top="props">
              <div class="tb-app col">
                <div class="filter-bar-wrapper accent-filter-bar">
                  ...
                  <!-- Matches the legacy list grid's "Export Grid" toolbar
                       button (tsbExportGrid_Click in BUK_BookingList.cs) —
                       exports the currently visible/filtered rows, purely
            ...
            <template v-slot:body-cell-action="props">
              <q-td :props="props" class="button-container">
                ...
                <q-btn
                  ...
                  @click="printRowBooking(props.row)"
                >
...
export default {
  ...
  methods: {
    ...
    // Matches the legacy list grid's "Export Grid" toolbar button
    // (tsbExportGrid_Click -> CF.DevXGridExport in BUK_BookingList.cs) —
    // exports the currently visible columns/rows to a CSV file client-side.
    // Purely read-only, no backend call (this app is mock-data only).
    exportGrid() {
      const cols = this.baseColumns.filter((c) => c.name !== "action");
      const header = cols.map((c) => c.label).join(",");
      const rows = this.filteredBookings.map((row) =>
        cols
          .map((c) => {
    ...
    async printRowBooking(row) {
      const data = await apiGetBookingById(row.BookingId);
      this.form = { ...data };
      this.printBooking(true);
    },
