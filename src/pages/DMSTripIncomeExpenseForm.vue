...
<script>
import ListPage from "./DMSTripIncomeExpense.vue";

export default {
  name: "DMSTripIncomeExpenseForm",
  extends: ListPage,
  ...
  async created() {
    ...
    try {
      if (this.dialogMode !== "add") {
        ...
        const record = this.allEntries.find(
          (row) => String(row.EntryId) === String(id)
        );
        if (!record)
          throw new Error(
  ...
  methods: {
    closeDialog() {
      this.backToEntryList("/DMSTripIncomeExpense", "Trip Income / Expense");
    },
  },
};
</script>
