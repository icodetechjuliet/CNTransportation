          ...
          >
            ...
            <template v-slot:item="props">
              <div class="mobile-job-card">
                <div class="mjc-header" @click="toggleMobileCard(props.row.ReceivedCollectionId)">
                  ...
                  <div class="mjc-header-right">
                    ...
                    <q-icon
                      :name="expandedMobileCards.includes(props.row.ReceivedCollectionId) ? 'expand_less' : 'expand_more'"
                      size="20px"
                      color="grey-6"
                    />
                  </div>
                ...
                <transition name="mobile-expand">
                  <div v-if="expandedMobileCards.includes(props.row.ReceivedCollectionId)" class="mjc-details">
...
const MOCK_VOUCHERS = [
  ...
  {
    ReceivedCollectionId: 2,
    VoucherNo: "RC20260002",
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      if (voucher.ReceivedCollectionId) {
        const idx = MOCK_VOUCHERS.findIndex((o) => o.ReceivedCollectionId === voucher.ReceivedCollectionId);
        if (idx !== -1) MOCK_VOUCHERS[idx] = { ...voucher };
      } else {
        voucher.ReceivedCollectionId = MOCK_VOUCHERS.length + 1;
...
function apiDeleteVoucher(id) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const idx = MOCK_VOUCHERS.findIndex((o) => o.ReceivedCollectionId === id);
...
function apiGetVoucherById(id) {
  return new Promise((resolve) => {
    setTimeout(
      () => resolve(MOCK_VOUCHERS.find((o) => o.ReceivedCollectionId === id) || null),
      100
    );
  });
}

// Consumed by DMSBookingOfficeWiseReceivedCollectionView.vue — its own
...
export default {
  ...
  methods: {
    ...
    openAddVoucher() {
      if (this.openTab) {
        this.openTab("/DMSBookingOfficeWiseReceivedCollectionView?mode=add", "New Received Collection");
        return;
      }
    ...
    confirmDeleteVoucher(row) {
      this.$q
        ...
        .onOk(async () => {
          ...
          this.$q.notify({ message: "Received Collection deleted", color: "negative", position: "top" });
          this.loadVouchers();
        });
    },
