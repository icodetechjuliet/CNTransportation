...
<script>
import ListPage from "./DMSTripCharge.vue";

export default {
  name: "DMSTripChargeForm",
  extends: ListPage,
  ...
  async created() {
    ...
    try {
      if (this.dialogMode !== "add") {
        ...
        const record = this.allCharges.find(
          (row) => String(row.ChargeId) === String(id)
        );
        if (!record)
          throw new Error(
  ...
  methods: {
    closeDialog() {
      this.backToEntryList("/DMSTripCharge", "Trip Charge");
    },
  },
};
</script>
